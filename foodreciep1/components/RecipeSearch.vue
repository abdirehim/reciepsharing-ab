<template>
  <div id="search">
    ...
    <div class="container mx-auto p-4 bg-gray-400 rounded-lg shadow-lg my-8">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4 items-center  ">
        ...
        <!-- Filter Icon -->
        <div class="relative">
          <button @click="filterDropdownOpen = !filterDropdownOpen" class="text-gray-700 focus:outline-none ml-20">
            <IconsFilter class="w-8 h-8" />
            <span class="sr-only">Filter</span>
          </button>
          <!-- Categories Dropdown -->
          <div v-if="filterDropdownOpen" class="absolute top-full left-0 mt-2 bg-white rounded-md shadow-lg z-10 w-48">
            <div class="block px-4 py-2 text-gray-800 hover:bg-gray-200 cursor-pointer" @click="applyFilter('')">
              All
            </div>
            <div v-for="category in categories" :key="category.id" class="block px-4 py-2 text-gray-800 hover:bg-gray-200 cursor-pointer" @click="applyFilter(category.id)">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 my-8">
        <div v-for="recipe in recipes" :key="recipe.id" class="bg-white p-4 rounded-lg shadow-lg hover:shadow-2xl transition-shadow duration-300 transform hover:scale-105">
          ...
          <div class="w-8 h-8 rounded-full bg-gray-400 flex items-center justify-center text-white font-bold">
              <span v-if="recipe.photoUrl">{{ user.photoUrl[0] }}</span>
              <span v-else>{{ recipe.user?.username.slice(0, 2).toUpperCase() }}</span>
            </div>
        </div>
      </div>
